...
namespace Nauplius.SharePoint.BlobCache.Layouts.Nauplius.SharePoint.BlobCache
{
    public partial class BlobCacheSettings : LayoutsPageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try { TbCacheSize.Text = currentModifications[MaxSize]; }
            catch (KeyNotFoundException) { TbCacheSize.Text = "10"; }

            try { TbCacheFilePath.Text = currentModifications[Location]; }
            catch (KeyNotFoundException) { TbCacheFilePath.Text = @"C:\BlobCache\14"; }
            ...
            try { TbCacheMaxAge.Text = currentModifications[MaxAge]; }
            catch (KeyNotFoundException) { }

            try { TbCacheChangeCheckInternval.Text = currentModifications[ChangeCheckInterval]; }
            catch (KeyNotFoundException) { }

            try { TbCachePolicyCheckInterval.Text = currentModifications[PolicyCheckInterval]; }
            catch (KeyNotFoundException) { }

            try
            {
                BtnBlobCacheEnable.Checked = Boolean.Parse(currentModifications[Enabled]);
                BtnBlobCacheDisable.Checked = !BtnBlobCacheEnable.Checked;
            }
            catch (KeyNotFoundException)
            {
            ...
            try { TbCacheWriteIndexInterval.Text = currentModifications[WriteIndexInterval]; }
            catch (KeyNotFoundException) { }

            try { TbCacheDeleteOldCacheFoldersInterval.Text = currentModifications[DeleteOldCacheFolders]; }
            catch (KeyNotFoundException) { }

            try { TbCacheDeleteCheckedInFilesInterval.Text = currentModifications[DeleteCheckedInFiles]; }
            catch (KeyNotFoundException) { }

            try { TbCacheLargeFileSize.Text = currentModifications[LargeFileSize]; }
            catch (KeyNotFoundException) { }

            try { TbCacheReadChunkSize.Text = currentModifications[ReadChunkSize]; }
            catch (KeyNotFoundException) { }

            try { TbCacheThreadThrottleLimit.Text = currentModifications[MaxConcurrentLargeRequests]; }
            catch (KeyNotFoundException) { }

            // Undocumented Attributes

            try { TbCacheBlobThrottlingThreshold.Text = currentModifications[BlobThrottlingThreshold]; }
            catch (KeyNotFoundException) { }

            try
            {
                BtnCacheBlobThrottlingEnable.Checked = Boolean.Parse(currentModifications[BlobThrottlingEnabled]);
            ...
            try
            {
                BtnCacheBlobThrottleAnonymousOnlyEnable.Checked = Boolean.Parse(currentModifications[BlobThrottleOnlyAnonymous]);
            ...
            try
            {
                BtnCacheBlobThrottleNonPublishingOnlyEnable.Checked = Boolean.Parse(currentModifications[BlobThrottleOnlyNonPublishing]);
